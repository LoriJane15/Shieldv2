...
class EclipReleaseAcknowledgmentStorageService
{
    public function store(UploadedFile $file, EclipCase $case): string
    {
        $path = $file->storeAs(
            "eclip/{$case->id}/releases",
            Str::uuid().'.'.strtolower($file->extension() ?: 'bin'),
            'local',
        );
        throw_if($path === false, \RuntimeException::class, 'The release acknowledgment could not be stored.');
