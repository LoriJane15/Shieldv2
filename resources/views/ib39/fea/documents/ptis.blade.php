<article class="sheet" data-form="ptis-main">
    ...
    <table class="official-table" style="margin-top:0"><tbody>
        <tr><th style="width:8%">TO</th><td>{{ $draft['to'] ?? '' }}</td><th style="width:34%">SUPPLY CLASSIFICATION<br>OFFICER</th><td>{{ $draft['supply_classification_officer'] ?? '' }}</td><th style="width:18%">PAGE ___ OF<br>Voucher Number</th><td>{{ $draft['page_of_voucher_number'] ?? '' }}</td></tr>
        <tr><th>FROM</th><td>{{ $draft['from'] ?? '' }}</td><th>Organization/ Unit:</th><td>{{ $draft['organization_unit'] ?? '' }}</td><th>Turn-In Slip Number:</th><td>{{ $draft['turn_in_slip_number'] ?? '' }}</td></tr>
    </tbody></table>
    ...
    <div class="line-grid"><strong>BASIS:</strong><span class="line-value value">{{ $draft['basis'] ?? '' }}</span></div>
    @foreach(['turned_in_by' => ['TURNED IN BY', '(Name, please specify if FR or FVE)'], 'received_by' => ['RECEIVED BY', '(signature above printed name)'], 'inspected_by' => ['INSPECTED BY', '(signature above printed name)']] as $key => [$label, $caption])
        <div class="signature-block" style="text-align:left;margin-left:0"><strong>{{ $label }}:</strong><div class="signature-line" style="margin-left:35mm"></div><div class="printed-name" style="margin-left:35mm;width:62mm;text-align:center">{{ $draft[$key] ?? '' }}</div><div class="signature-caption" style="margin-left:35mm;width:62mm;text-align:center">{{ $caption }}</div></div>
    @endforeach
...
<article class="sheet continuation" data-form="ptis-continuation">
    ...
    <div class="legend-grid">
        ...
        <div>
            <div class="certification-box">I HEREBY CERTIFY that the article/s listed herein are turned-in under the circumstances indicated therein:<br><br><strong>FOR THE COMMANDING OFFICER:</strong><div class="signature-line"></div><div class="printed-name" style="text-align:center">{{ $draft['commanding_officer'] ?? '' }}</div><div style="text-align:center">(signature above printed name)<br><strong>AFP/ PNP Representative</strong></div><br><strong>CONFIRMED BY:</strong><div class="signature-line"></div><div class="printed-name" style="text-align:center">{{ $draft['dilg_representative'] ?? '' }}</div><div style="text-align:center">(signature above printed name)<br><strong>(DILG Representative)</strong></div></div>
