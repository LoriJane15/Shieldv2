...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div>
                <h1 class="card-title fs-3 mb-1">Good Day! <span class="text-primary">{{ $userFullname }}</span></h1>
        ...
        <div class="col-md-4">
            <div class="card h-100">
                <div class="card-body">
                    ...
                    @forelse ($recentDocuments->take(3) as $doc)
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="text-muted">RCSP barangay status distribution for {{ $municipalName }}.</p>
                    <div style="height:280px;"><canvas id="rcspBarChart"></canvas></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
(function () {
    const labels = @json($statusChart->keys());
    const values = @json($statusChart->values());
    new Chart(document.getElementById('rcspBarChart'), {
        type: 'bar',
        data: { labels, datasets: [{ label: 'RCSP Barangays', data: values,
            backgroundColor: ['#f0ad4e', '#2196f3', '#28a745'], borderRadius: 6, maxBarThickness: 90, categoryPercentage: 0.6, barPercentage: 0.7 }] },
